...
- 试玩中发现 Agent 工具路由过宽：普通角色扮演请求里的“检查一下”会误触发联网搜索，“今晚”会误触发当前时间工具。已收窄 `shouldUseSearchTool` / `shouldUseTimeTool`，并让策略层复用同一套 detector，避免创作陪伴场景被错误归类成资料查询。
- `npm run test:agent` 已新增并通过“roleplay check does not trigger web search or clock”与 detector 回归用例，防止这类误判复发。
