...
$ npm run lint                  # 通过
$env:VITE_BASE_PATH='/yuri_chat/'; $env:VITE_API_BASE_URL=(Get-Content -Raw .\secrets\cloud-api-url.txt).Trim(); npm run build
...
- `package.json` 包名、`src/config/brand.ts`、Pages base path、文档、后端默认 SQLite 文件名、systemd 文档和 `YURI_CHAT_*` 环境变量已改到 `yuri_chat`。
- `src/config/storage.ts` 里的 `databaseName: 'yuri-nest'` 故意保留，登录 token / 云端口令 key 已切到 `yuri_chat-*`，并兼容读取旧 `yuri-chat-*`、`yuri-nest-cloud-token`，避免妹妹本机 IndexedDB 与 localStorage 数据不可见。
- `src/data/migrations.ts` 状态版本升到 35，会把旧默认群聊 id `room-yuri-nest` / `room-yuri-chat` 迁到 `room-yuri_chat`，也会把旧项目记忆 scope 迁到 `yuri_chat`，保留原消息并去重。
...
npm run build          # VITE_BASE_PATH=/yuri_chat/，dist/index.html 已确认 /yuri_chat/assets/...
```

浏览器实测：

- 本地 Vite `http://127.0.0.1:5177/yuri_chat/` 打开标题为“百合小窝 · yuri_chat”。
- 页面发出一条聊天消息，`/api/chat` 返回本地兜底回复。
- 浏览器内 fetch `/api/model/profiles` 返回 200 且有服务器默认配置；fetch `/api/cloud/state` 返回 200。
- IndexedDB 数据库名仍为 `yuri-nest`，localStorage 旧 key `yuri-nest-cloud-token` / `yuri-chat-*` 可读，并会迁移到新的 `yuri_chat-*` key。
...
$env:VITE_BASE_PATH='/yuri_chat/'; $env:VITE_API_BASE_URL=(Get-Content -Raw .\secrets\cloud-api-url.txt).Trim(); npm run build
                              # 通过，dist 已生成 /yuri_chat/ base path 的分包产物
```
