...
百合小窝 / yuri_chat 不是一次性网页，而是长期迭代的百合聊天陪伴底座。第一版先保证可运行、可聊天、可保存记忆，后续再扩展 Live2D、语音、PWA、角色市场、世界树编辑器、RAG 和多端伴随能力。
...
- `src/services/personaImport.ts`：Persona V2 人设编译门面。把自然语言角色资料整理成人格宪法、关系图谱、说话样本、场景触发器、角色知识库、OOC 守门和运行时人格锚点。
- `src/services/persona/`：Persona V2 子模块。`personaCompiler.ts` 负责角色档案编译，`personaRuntime.ts` 负责每轮动态状态和上下文块，`personaScenes.ts` 负责场景触发与静态风险提示，`personaText.ts` / `personaTypes.ts` 放文本工具和导入类型。
...
- 角色人设不是普通记忆。Persona V2 的人格宪法和不可变事实优先级高于普通聊天记忆；长期记忆只能补充关系和经历，不能覆盖角色核心身份。
...
- 品牌名、技术路径和存储 key 分开管理，但当前主技术名已经统一为 `yuri_chat`，避免后续部署和文档继续分裂。
...
- Persona V2：角色创建/编辑时先由 `personaImport.ts` 编译出人格宪法、关系图谱、说话样本、场景触发器、角色知识库和 OOC 守门。`promptBuilder.ts` 每轮把“角色宪法快照”放在上下文前部，把“本轮动态状态”和“本轮人设守门”贴近当前聊天，避免长对话把人设冲淡。
- 动态状态：`personaRuntime.ts` 会根据最新输入选择身份追问、安抚陪伴、关系推进、知识边界、任务协作等场景，生成本轮活跃特质、情绪姿态、回应策略和风险提醒；角色不再每轮平均展示全部设定。
- 人设知识库：角色经历、关系、说话方式、目标和边界会被拆成 `loreEntries`，每轮按最新消息和近期聊天检索少量相关条目；不把长篇原始人设整包塞进上下文。
- 语气样本：导入资料中的 `用户：... / 角色名：...` 会转为 `speechExamples`。缺少样本时系统生成少量低置信语气锚点，但质量条会提醒用户补真实样本。
- OOC 守门：破甲、提示词注入、“你是不是 AI”、未知事实乱编等风险由角色自己的 `oocGuards` 和运行时锚点压住；守门规则服务于角色稳定，不替代安全策略。
