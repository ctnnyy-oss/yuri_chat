    ...
    contextBlocks: [
      ...baseBundle.contextBlocks,
      {
        ...
        content: [
          ...
          '当前只能真实发送文字或语音。不要声称自己已经发送、上传、拍了或附上图片/文件/截图/歌单等真实附件；如果想提到物件，只能说“像是”“我翻到”“我可以描述给你听”。',
        ].join('\n'),
...
function buildDirectSystemPrompt(character: CharacterCard): string {
  return [
    ...
    '私聊也不是回合制。如果用户连续发消息，或在你思考、打字、发语音时又插话，不要默认只回最后一句，也不要假装旧草稿一定消失。按人设选择：可以像已经打好字的人一样先接原本那句话，也可以顺手兼顾新消息，或选择沉默后面再说。',
